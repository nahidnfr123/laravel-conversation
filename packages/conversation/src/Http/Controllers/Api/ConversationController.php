...
class ConversationController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        return auth()->user();
        // Favorite
        $favorite = Conversation::inFavorite($request['id']);

        // User data
        if ($request['type'] == 'user') {
            $fetch = User::where('id', $request['id'])->first();
            if ($fetch) {
                $userAvatar = Conversation::getUserWithAvatar($fetch)->avatar;
            }
        }
    ...
    public function download($fileName)
    {
        ...
        if (Conversation::storage()->exists($path)) {
            return response()->json([
                'file_name' => $fileName,
                'download_path' => Conversation::storage()->url($path)
            ], 200);
        } else {
            return response()->json([
    ...
    public function send(Request $request): JsonResponse
    {
        ...
        if ($request->hasFile('file')) {
            // allowed extensions
            $allowed_images = Conversation::getAllowedImages();
            $allowed_files = Conversation::getAllowedFiles();
            $allowed = array_merge($allowed_images, $allowed_files);

            $file = $request->file('file');
            // check file size
            if ($file->getSize() < Conversation::getMaxUploadSize()) {
        ...
        if (!$error->status) {
            ...
            $messageData = Conversation::parseMessage($message);

            // send to user using pusher
            if (Auth::user()->id != $request['id']) {
                Conversation::push("private-messages." . $request['id'], 'messaging', [
                    'from_id' => Auth::user()->id,
                    'to_id' => $request['id'],
                    'message' => $messageData
                ]);
